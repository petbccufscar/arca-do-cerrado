...
const SpecieCard = ({ specie }) => {
    const firstImage = specie.imagens.length > 0 ? specie.imagens[0].imagem : 'placeholder.jpg'; // Adiciona um placeholder de imagem
    return (
        <div className="w-72 h-100 bg-white border border-gray-200 rounded-lg shadow overflow-hidden">
            <a href={`/especies/${specie.id}`}>
                <img className="rounded-t-lg h-40 w-full object-cover" src={firstImage} alt={specie.apelido} /> {/* Adiciona o atributo alt */}
            </a>
