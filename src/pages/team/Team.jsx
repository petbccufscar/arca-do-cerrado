...
const Person = ({ personName, personImage, personRole, personId }) => {
	return (
				...
				<div className='flex flex-col py-8 px-6 mx-auto max-w-screen-xl lg:px-8  items-center'>
					...
					<div className='grid gap-8 lg:gap-16 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4'>
						...
						{teamData.map((person, index) => (
							<Person key={index} personId={person.id} personName={person.nome} personLink={person.link} personImage={person.imagem} personRole={person.cargo} />
						))}
					</div>
				</div>
			</section>
